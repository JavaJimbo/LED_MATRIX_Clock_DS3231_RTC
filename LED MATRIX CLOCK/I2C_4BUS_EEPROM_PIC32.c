...
{                   
    ...
    I2CSetFrequency(busID, SYS_FREQ, 100000);
...
/* Reads a block of bytes from DS3231 beginning at startAddress
 *	Inputs:
 *	device - I2C address of the chip
 *	startAddress - 16-bit memory address to write the data to 	
 *	numBytes - numBytes - Number of bytes to be written
...
unsigned char DS3231ReadBlock(unsigned char busID, unsigned char startAddress, unsigned char *ptrData, unsigned char numBytes) 
{
    ...
    if (!TransmitOneByte(busID, DS3231_TIMEKEEPER | WR)) return 0; // Send WRITE command and I2C device address
    if (I2CGetACK(busID)) return 0; // Get ACK from EEPROM
    //if (I2CSTATbits.ACKSTAT) return (0); // Get ACK from EEPROM

    if (!TransmitOneByte(busID,  startAddress)) return 0; // Send EEPROM high address byte
    if (I2CGetACK(busID)) return 0; // Get ACK from EEPROM
    //if (I2CSTATbits.ACKSTAT) return (0); // Get ACK from EEPROM    
    ...
    if (!TransmitOneByte(busID, DS3231_TIMEKEEPER | RD)) return 0; // Now send ID with READ Command    
    if (I2CGetACK(busID)) return 0; // Get ACK from EEPROM
    // if (I2CSTATbits.ACKSTAT) return (0); // Get ACK from EEPROM
...
unsigned char DS3231WriteByte(unsigned char busID, unsigned char address, unsigned char data)
{
    ...
    if (!TransmitOneByte(busID, DS3231_TIMEKEEPER | WR)) return 0;
    if (I2CGetACK(busID)) return 0; // Get ACK from EEPROM
    // if (I2CSTATbits.ACKSTAT) return (0); // Get ACK from EEPROM    

    if (!TransmitOneByte(busID, address)) return 0; // Send EEPROM low address byte
    if (I2CGetACK(busID)) return 0; // Get ACK from EEPROM
    // if (I2CSTATbits.ACKSTAT) return (0); // Get ACK from EEPROM    
...
unsigned char DS3231WriteBlock(unsigned char busID, unsigned char startAddress, unsigned char *ptrData, unsigned short numBytes) 
{
    ...
    if (!TransmitOneByte(busID, startAddress)) return 0;
    if (I2CGetACK(busID)) return 0; // Get ACK from EEPROM
    //if (I2CSTATbits.ACKSTAT) return (0); // Get ACK from EEPROM    
